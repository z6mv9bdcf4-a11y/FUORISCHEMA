...
(() => {
  ...
  function showOwnerToast(message, isError = false) {
    ...
    if (!toast) {
      ...
      toast.style.cssText = `
        ...
        border-radius:999px;background:rgba(12,12,14,.94);
        color:#fff;font:800 10px Inter,Arial,sans-serif;
  ...
  function cleanModerationPage() {
    ...
    const observer = new MutationObserver(() => {
      const panel = document.getElementById("fsocialPostManagement");
  ...
  function addOwnerDeleteButton(article) {
    ...
    const postId = Number(article.querySelector("[data-post-id]")?.dataset.postId || article.dataset.postId || 0);
    if (!postId) return;
    ...
    button.style.cssText = `
      margin-left:auto;padding:6px 10px;border:1px solid rgba(255,59,48,.32);
      border-radius:7px;color:#ff6258;background:rgba(255,59,48,.035);
      font:900 9px Inter,Arial,sans-serif;letter-spacing:1px;
  ...
  function addOwnerToolsToFeed() {
    ...
    const scan = () => {
      feed.querySelectorAll(".post-card").forEach(addOwnerDeleteButton);
    };
