...
(() => {
  ...
  .fs-final-public #fsProfileSaved,.fs-final-public #fsProfileSettings{display:none!important}
  ...
  async function socialSettings(){
    const u=await user();
    if(!u)return;
    const {data}=await supabase.from('profile_settings').select('message_privacy,show_activity').eq('user_id',u.id).maybeSingle();
    const s=data||{message_privacy:'everyone',show_activity:true};
    openSheet('IMPOSTAZIONI SOCIAL',`<div class="fpv2-setting"><div><strong>Messaggi</strong><span>Chi può iniziare una conversazione con te.</span></div><select class="fpv2-select" data-message-privacy><option value="everyone" ${s.message_privacy==='everyone'?'selected':''}>TUTTI</option><option value="nobody" ${s.message_privacy==='nobody'?'selected':''}>NESSUNO</option></select></div><div class="fpv2-setting"><div><strong>Attività</strong><span>Permetti agli altri di vedere la tua attività.</span></div><button class="fpv2-toggle ${s.show_activity?'on':''}" data-toggle="activity" type="button"><i></i></button></div><button class="fpv2-save" data-save-settings type="button">SALVA IMPOSTAZIONI SOCIAL</button>`,root=>{
      ...
      root.querySelector('[data-save-settings]').onclick=async()=>{
        ...
        setTimeout(()=>root.querySelector('.fpv2-save').textContent='SALVA IMPOSTAZIONI SOCIAL',1200);
      };
    });
  }
  ...
  function removePersonalControls(){
    ['fsProfileSaved','fsProfileSettings'].forEach(id=>document.getElementById(id)?.remove());
  }
  ...
  function mount(){
    ...
    if(!document.getElementById('fsProfileSettings')){
      const b=document.createElement('button');
      b.id='fsProfileSettings';b.className='action-btn';b.textContent='IMPOSTAZIONI SOCIAL';b.onclick=socialSettings;actions.appendChild(b);
    }
  }
